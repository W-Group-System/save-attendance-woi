...
class spai_store_attendance extends Command
{
    /**
    ...
    public function handle()
    {
        ...
        info("START Get Attendance Store in SPAI");
        ...
        if($attendance == null)
        {
            ...
            $attendances = AttendanceLog::orderBy('id','asc')->where('location','SPAI')->take(100)->get();
        }
        else
        {
            // $attendances = AttendanceLog::where('id','>',$attendance->last_id)->where('location','!=','System')->orderBy('id','asc')->get()->take(100);
            $attendances = AttendanceLog::where('id','>',$attendance->last_id)->where('location','SPAI')->orderBy('id','asc')->take(100)->get();
        }
        ...
        info("End Get Attendance Store in SPAI");
    }
}

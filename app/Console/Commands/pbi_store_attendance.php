...
class pbi_store_attendance extends Command
{
    /**
    ...
    public function handle()
    {
        ...
        info("START Get Attendance Store in PBI");
        ...
        if($attendance == null)
        {
            ...
            $attendances = AttendanceLog::orderBy('id','asc')->where('location','PBI')->take(100)->get();
        }
        else
        {
            // $attendances = AttendanceLog::where('id','>',$attendance->last_id)->where('location','!=','System')->orderBy('id','asc')->get()->take(100);
            $attendances = AttendanceLog::where('id','>',$attendance->last_id)->where('location','PBI')->orderBy('id','asc')->take(100)->get();
        }
        ...
        info("End Get Attendance Store in PBI");
    }
}

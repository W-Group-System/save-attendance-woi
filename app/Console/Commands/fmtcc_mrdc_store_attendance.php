...
class fmtcc_mrdc_store_attendance extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'command:fmtcc_mrdc_store_attendance';
    ...
    public function handle()
    {
        ...
        if($attendance == null)
        {
            $attendances = AttendanceLog::orderBy('id','asc')->where('location','FMTCC/MRDC')->take(100)->get();
        }
        else
        {
            $attendances = AttendanceLog::where('id','>',$attendance->last_id)->where('location','FMTCC/MRDC')->orderBy('id','asc')->take(100)->get();
        }

...
class pri_store_attendance extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'command:pri_store_attendance';
    ...
    public function handle()
    {
        ...
        info("START Get Attendance Store in PRI");
        ...
        if($attendance == null)
        {
            ...
            $attendances = AttendanceLog::orderBy('id','asc')->whereIn('location',['PRI', 'PRI-BMO'])->take(100)->get();
        }
        else
        {
            // $attendances = AttendanceLog::where('id','>',$attendance->last_id)->where('location','!=','System')->orderBy('id','asc')->get()->take(100);
            $attendances = AttendanceLog::where('id','>',$attendance->last_id)->whereIn('location',['PRI', 'PRI-BMO'])->orderBy('id','asc')->take(100)->get();
        }
        ...
        info("End Get Attendance Store in PRI");
    }
}

...
class wfa_store_attendance extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'command:wfa_store_attendance';
    ...
    public function handle()
    {
        ...
        info("START Get Attendance Store in WFA");
        ...
        if($attendance == null)
        {
            ...
            $attendances = AttendanceLog::orderBy('id','asc')->where('location','WFA')->take(100)->get();
        }
        else
        {
            // $attendances = AttendanceLog::where('id','>',$attendance->last_id)->where('location','!=','System')->orderBy('id','asc')->get()->take(100);
            $attendances = AttendanceLog::where('id','>',$attendance->last_id)->where('location','WFA')->orderBy('id','asc')->take(100)->get();
        }
        ...
        info("End Get Attendance Store in WFA");
    }
}

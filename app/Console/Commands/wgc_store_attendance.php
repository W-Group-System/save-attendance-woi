...
class wgc_store_attendance extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'command:wgc_store_attendance';
    ...
    public function handle()
    {
        ...
        info("START Get Attendance Store in WGC");
        ...
        if($attendance == null)
        {
            ...
            $attendances = AttendanceLog::orderBy('id','asc')->where('location','WGC')->take(100)->get();
        }
        else
        {
            // $attendances = AttendanceLog::where('id','>',$attendance->last_id)->where('location','!=','System')->orderBy('id','asc')->get()->take(100);
            $attendances = AttendanceLog::where('id','>',$attendance->last_id)->where('location','WGC')->orderBy('id','asc')->take(100)->get();
        }
        ...
        info("End Get Attendance Store in WGC");
    }
}
